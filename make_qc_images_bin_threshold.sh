...
not_in_knox_outdir_extension="/allen_api_not_in_knox/"
...
declare -A file_to_outdir=(
    ["$csv_file_not_in_knox_exps"]="$not_in_knox_outdir_extension"
    #["$csv_file_main_exps"]=""
...
for csv_file in "${!file_to_outdir[@]}"; do
     ...
     mkdir -p "${qc_jpg_dir_inj}${outdir_extension}"
     mkdir -p "${qc_jpg_dir_proj}${outdir_extension}"

     for tracer in "${csv_data[@]}"; do  
          ##change threshold from 0.01 as needed
          inj_file_thresh="${inj_dir}${outdir_extension}/${tracer}/injection_dens_times_frac_bin${inj_thresh}.mnc"
          proj_file_thresh="${proj_dir}${outdir_extension}/${tracer}/projection_density_bin${proj_thresh}.mnc"
...
tracers_proj=("100148443" "100148554" "112460257" "114045733" "114400640" "125801739" "126115436" "126352037" "127085717" "127711098" "141601779" "146012184" "158738894" "180568155" "272970039")
qc_jpg_dir_proj="../derivatives/knox_proj/bin${proj_thresh}/cropped/"
mkdir -p "${qc_jpg_dir_proj}"

tracers_inj_not_in_knox=("640285199")

for tracer in "${tracers_inj_not_in_knox[@]}"; do
 #replace inj/proj as needed
     inj_file_thresh="${inj_dir}/allen_api_not_in_knox/${tracer}/injection_dens_times_frac_bin${inj_thresh}.mnc"
