...
EXPERIMENTS_EXCLUDE_JSON = os.path.join(TOP_DIR, "experiments_exclude_updated_101825.json")
...
def main():
    ...
    get_metric('connection_density').to_csv(
        os.path.join(output_dir, 'connection_density_rebuilt_1018.csv'))
    get_metric('connection_strength').to_csv(
        os.path.join(output_dir, 'connection_strength_rebuilt_1018.csv'))
    get_metric('normalized_connection_density').to_csv(
        os.path.join(output_dir, 'normalized_connection_density_rebuilt_1018.csv'))
    get_metric('normalized_connection_strength').to_csv(
        os.path.join(output_dir, 'normalized_connection_strength_rebuilt_1018.csv'))
    ...
    np.savetxt(os.path.join(output_dir, 'weights_rebuilt_1018.csv.gz'),
               weights.astype(np.float32), delimiter=',')
    np.savetxt(os.path.join(output_dir, 'nodes_rebuilt_1018.csv.gz'),
