...
##load Allen experiments that were not in Knox (will still be included in manual/automated QC)
allen_exps_not_in_knox <- as.data.frame(read.csv("knox_experiment_csvs/wt_experiments_allen_api_not_knox.csv"))
tracer_list_not_in_knox <- unlist(allen_exps_not_in_knox)
###################################################
...
tracer_dir_not_in_knox <- "../preprocessed/knox_connectome_tracers/allen_api_not_in_knox/"
...
tracer_oob_vent_prop_df_not_in_knox <-  foreach(
  tracer = tracer_list_not_in_knox, 
  .combine = rbind, 
  .export = c("calculate_size_oob_vent_voxels_thresh", "inj_thresh", "proj_thresh", "tracer_dir_not_in_knox", "csf_voxels", "mincGetVolume", "maskVol")
  
) %dopar% {
  tempMatrix <- calculate_size_oob_vent_voxels_thresh(tracer_dir_not_in_knox, tracer, inj_thresh, proj_thresh, csf_voxels)
  tempMatrix
}
#stop cluster
stopCluster(cl)
...
colnames(tracer_oob_vent_prop_df_not_in_knox) <- c("tracer", "proj_num_vox", "proj_vox_oob", "proj_vox_vent", 
                                       "inj_num_vox", "inj_vox_oob", "inj_vox_vent")
...
write.csv(tracer_oob_vent_prop_df_not_in_knox, paste0("tables/knox_oob_vent_df_inj",inj_thresh,"_proj",proj_thresh,"_allen_api_not_in_knox.csv"))

##create merged automated QC csv for overall exclusion/comparison to manual
tracer_oob_vent_prop_df <- as.data.frame(read.csv(paste0("tables/knox_oob_vent_df_inj",inj_thresh,"_proj",proj_thresh,".csv")))
tracer_oob_vent_prop_df_not_in_knox <- as.data.frame(read.csv(paste0("tables/knox_oob_vent_df_inj",inj_thresh,"_proj",proj_thresh,"_allen_api_not_in_knox.csv")))
tracer_num_vox_oob_vent_df_full <- rbind(tracer_oob_vent_prop_df, tracer_oob_vent_prop_df_not_in_knox)
colnames(tracer_num_vox_oob_vent_df_full) <- colnames(tracer_oob_vent_prop_df)
write.csv(tracer_num_vox_oob_vent_df_full, paste0("./tables/knox_oob_vent_df_inj",inj_thresh,"_proj",proj_thresh,"_437exps.csv")) 