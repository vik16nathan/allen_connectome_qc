...
allen_api_exps_not_in_knox <- as.data.frame(read.csv("knox_experiment_csvs/wt_experiments_allen_api_not_knox.csv"))
...
knox_experiments_included <- rbind(knox_experiments_included, allen_api_exps_not_in_knox)
...
knox_joined_full <- knox_experiments_included %>% left_join(tracer_sites %>% select(id, `structure-id`),
            by = c("id" = "id"))
...
tracer_list_not_in_knox <- as.data.frame(read.csv("./knox_experiment_csvs/wt_experiments_allen_api_not_knox.csv"))
for(tracer in tracer_removal_df$Tracer)  {
  if(tracer %in% allen_api_exps_not_in_knox$id) {
     projection_density_bin0.1 <- mincArray(mincGetVolume(paste0(allen_tracer_dir, "/allen_api_not_in_knox/",tracer,"/projection_density_bin0.1.mnc")))
  }
  else {
